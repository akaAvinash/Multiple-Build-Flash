...
def flash_adb_devices(adb_devices):
    ...
    for device in adb_devices:
        cwd = os.getcwd()
        terminal_command = f'gnome-terminal -- bash -c "cd {cwd} && ./{flash_filename} --aserial {device} --fserial {device}; sleep 10; export ANDROID_SERIAL={device}; adb -s {device} wait-for-device; cd apps; adb -s {device} push *.{app_extension} /data/; exec bash"'
...
def flash_fastboot_devices(fastboot_devices):
    ...
    for idx, device in enumerate(fastboot_devices, start=1):
        ...
        terminal_command = f'gnome-terminal -- bash -c "cd {cwd} && ./{flash_filename} --fserial {fastboot_serial} && adb -s {fastboot_serial} push apps /data/; exec bash"'
        subprocess.Popen(terminal_command, shell=True).wait()
